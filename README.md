...
The server runs over HTTPS using a locally-trusted certificate generated by [mkcert](https://github.com/FiloSottile/mkcert). This encrypts all traffic including your password and session token.
...
### 2. Create the local CA and generate certificates

```powershell
mkcert -install
$ip = (Get-NetIPAddress -AddressFamily IPv4 -InterfaceAlias "Wi-Fi").IPAddress
mkdir server\certs
mkcert -cert-file server/certs/cert.pem -key-file server/certs/key.pem $ip localhost 127.0.0.1
```

### 3. Trust the CA on your iPhone (one time)

```powershell
Copy-Item "$(mkcert -CAROOT)\rootCA.pem" "$env:USERPROFILE\OneDrive\Desktop\rootCA.crt"
```

Email `rootCA.crt` to yourself, open it on your iPhone, then:
- **Settings → General → VPN & Device Management** → tap the profile → Install
- **Settings → General → About → Certificate Trust Settings** → toggle mkcert on
...
### 4. Browser compatibility on iOS

Use **Safari** on iPhone. Chrome and Brave on iOS do not honor user-installed root CA certificates, so they will block the connection. Safari fully respects the iOS certificate trust store.

**Tip:** In Safari, tap Share → "Add to Home Screen" to install it as a full-screen app icon with no browser chrome.
